...
inline void uart_print_number_base(int32_t n, const char *base, const uint8_t base_len) {
    if (n == INT32_MIN) {
...
inline void uart_print_unsigned_number_base(uint32_t n, const char *base, const uint8_t base_len) {
    if (n >= base_len) {
        uart_print_number_base(n / base_len, base, base_len);
    }
